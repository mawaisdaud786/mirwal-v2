...
test('seller customers are real, internally consistent, and isolated between sellers', async () => {
  ...
  assert.equal(alpha.body.data.summary.totalCustomers, alpha.body.data.customers.length)
  assert.equal(beta.body.data.summary.totalCustomers, beta.body.data.customers.length)

  // The summary total must equal the sum of each customer's own total — no double-counting
  // or drift between the aggregate and the per-row figures it is supposed to summarise.
  const expectedTotal = alpha.body.data.customers.reduce((sum, c) => sum + Number(c.totalSpent.amount), 0)
  assert.equal(Number(alpha.body.data.summary.totalSpent.amount), expectedTotal)
})
