...
function MobileBottomNav() { return <nav className="mobile-bottom-nav" aria-label="Mobile navigation">{[['house', 'Home', '/'], ['grip', 'Explore', '/explore'], ['tag', 'Deals', '/deals'], ['scale-balanced', 'Compare', '/compare'], ['user', 'Account', '/profile']].map(([icon, label, path]) => <button type="button" key={label} onClick={() => navigateTo(path)}><i className={`fa-solid fa-${icon}`} aria-hidden="true" /><span>{label}</span></button>)}</nav> }
export default function HomePage({ onAddToCart }) { return <><Header /><Hero /><TrustStrip /><Stats /><Categories /><AdSlot /><ProductSection title="Top Picks for You" sub="Handpicked products based on popularity and value" items={products.slice(0,5)} link="View All Deals" onAddToCart={onAddToCart} variant="top-picks" /><ProductSection title="More Products You'll Love" items={products.slice(5)} link="View All Products" onAddToCart={onAddToCart} variant="more-products" /><AdSlot /><Deals /><HowWorks /><Stores /><Reviews /><Newsletter /><Footer /><MobileBottomNav /></> }
