...
import { formatMinorUnits } from '@mirwal/shared/money'
...
export function AddToCartModal({ product, onClose, onViewCart }) {
  return <ModalFrame title="Added to cart" onClose={onClose} className="success-cart-modal"><div className="modal-status-icon success"><FaIcon name="check" /></div><h2>Added to cart!</h2><p>{product.name} has been added<br />to your cart successfully.</p><div className="modal-product-row"><img src={product.image} alt={product.name} /><span><b>{product.name}</b><small>{product.subtitle}</small></span><strong>{formatMinorUnits(product.unitMinor)}</strong></div><div className="modal-actions"><button type="button" onClick={onClose}><FaIcon name="arrow-left" /> Continue Shopping</button><button className="modal-primary" type="button" onClick={onViewCart}><FaIcon name="cart-shopping" /> Go to Cart</button></div></ModalFrame>
}
...
export function SuccessModal({ orderId = '#MW12345678', onClose, onViewOrders }) {
  return <ModalFrame title="Order placed successfully" onClose={onClose} className="success-order-modal"><div className="success-order-art"><FaIcon name="bag-shopping" /><span>Order Placed! <FaIcon name="heart" /></span></div><h2>Thank You!</h2><p>Your order has been placed successfully!<br /><small>We&apos;ve received your order and will keep you updated.<br />You&apos;ll get a confirmation email shortly.</small></p><div className="success-order-summary"><FaIcon name="truck" /><span><b>Order {orderId}</b><small>Placed successfully</small></span><FaIcon name="chevron-right" /></div><div className="modal-actions"><button type="button" onClick={onClose}>Continue Shopping</button><button className="modal-primary" type="button" onClick={onViewOrders}><FaIcon name="truck" /> Track Order</button></div><small className="success-order-footer"><FaIcon name="heart" /> Thanks for choosing Mirwal!</small></ModalFrame>
}
