...
export default function AccountShell({ path, cartCount, onAddToCart }) {
  let content
  if (path === '/orders') content = <OrdersContent />
  else if (path === '/my-returns') content = <MyReturnsPage />
  else if (['/track-orders', '/compare', '/compare-history', '/addresses', '/payment-methods', '/notifications', '/security', '/logout'].includes(path)) content = <AccountUtilityContent path={path} />
  else content = <AssistantRelatedContent path={path} onAddToCart={onAddToCart} />

  const active = path === '/my-returns' ? 'Returns' : path === '/compare-history' ? 'Compare History' : path === '/ai-assistant' ? '' : path === '/track-orders' ? 'Track Orders' : path === '/payment-methods' ? 'Payment Methods' : path === '/recently-viewed' ? 'Recently Viewed' : path === '/saved-searches' ? 'Saved Searches' : path === '/my-chats' ? 'My Chats' : path === '/wishlist' ? 'Wishlist' : path === '/profile' ? 'Profile' : path === '/orders' ? 'Orders' : path === '/compare' ? 'Compare' : path === '/addresses' ? 'Addresses' : path === '/notifications' ? 'Notifications' : path === '/security' ? 'Security' : 'Settings'
  ...
  return <div className="account-shell"><Header cartCount={cartCount} /><div className="account-shell-layout"><AccountSidebar active={active} /><div className={`account-shell-content account-page-shell${path === '/profile' ? ' account-profile-shell' : ''}`} onClick={handleAccountClick}>{content}</div></div></div>
}
